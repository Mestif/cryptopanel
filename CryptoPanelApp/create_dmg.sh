#!/bin/bash

# Скрипт для создания DMG файла для распространения

set -e

PROJECT_NAME="CryptoPanelApp"
APP_NAME="CryptoPanelApp.app"
DMG_NAME="CryptoPanel"
VERSION="1.0.0"
BUILD_DIR="build"
DMG_DIR="dmg_build"
DMG_FILE="${DMG_NAME}_v${VERSION}.dmg"

echo "📦 Создание DMG файла для $PROJECT_NAME..."

# Проверяем, что приложение собрано
if [ ! -d "$BUILD_DIR/$APP_NAME" ]; then
    echo "❌ Приложение не найдено. Сначала выполните: ./build.sh"
    exit 1
fi

# Создаем временную директорию для DMG
echo "📁 Создаю временную директорию..."
rm -rf "$DMG_DIR"
mkdir -p "$DMG_DIR"

# Копируем приложение
echo "📋 Копирую приложение..."
cp -R "$BUILD_DIR/$APP_NAME" "$DMG_DIR/"

# Создаем символическую ссылку на Applications
echo "🔗 Создаю ссылку на Applications..."
ln -s /Applications "$DMG_DIR/Applications"

# Настраиваем размер окна DMG
echo "⚙️  Настраиваю DMG..."

# Создаем DMG файл
echo "💿 Создаю DMG файл..."
hdiutil create -srcfolder "$DMG_DIR" \
    -volname "$DMG_NAME" \
    -fs HFS+ \
    -fsargs "-c c=64,a=16,e=16" \
    -format UDRW \
    -size 10m \
    "${DMG_FILE}.temp.dmg" || {
    echo "❌ Ошибка создания DMG"
    exit 1
}

# Настраиваем вид DMG (опционально, можно пропустить при ошибках)
echo "📐 Настраиваю вид DMG..."
sleep 1

# Пытаемся настроить вид через AppleScript
VOLUME_NAME="CryptoPanel"
osascript -e "tell application \"Finder\" to close window \"$VOLUME_NAME\"" 2>/dev/null || true

# Пропускаем детальную настройку вида (можно настроить вручную)
echo "ℹ️  DMG создан. Вид можно настроить вручную при необходимости."

# Конвертируем в финальный формат (сжатый, только для чтения)
echo "🗜️  Сжимаю DMG..."
hdiutil convert "${DMG_FILE}.temp.dmg" \
    -format UDZO \
    -imagekey zlib-level=9 \
    -o "$DMG_FILE" || {
    echo "❌ Ошибка сжатия DMG"
    exit 1
}

# Удаляем временный файл
rm -f "${DMG_FILE}.temp.dmg"

# Удаляем временную директорию
rm -rf "$DMG_DIR"

echo ""
echo "✅ DMG файл успешно создан!"
echo "📦 Файл: $DMG_FILE"
echo "📊 Размер: $(du -h "$DMG_FILE" | cut -f1)"
echo ""
echo "Готово к распространению! 🎉"

