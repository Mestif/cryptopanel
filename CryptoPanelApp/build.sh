...
# Подписываем приложение (adhoc signing для обхода Gatekeeper)
echo "🔐 Подписываю приложение..."
codesign --force --deep --sign - "$BUILD_DIR/$APP_NAME" || {
    echo "⚠️  Предупреждение: Не удалось подписать приложение. Это нормально для локальной сборки."
}
